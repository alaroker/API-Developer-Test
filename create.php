...
if(empty($input_saleorrent)){
    ...
    if(empty($county_err) && empty($country_err) && empty($town_err) && empty($description_err) && empty($details_err) && empty($address_err) && empty($image_err) && empty($latitude_err) && empty($longitude_err) && empty($bedrooms_err) && empty($bathrooms_err) && empty($price_err) && empty($type_err) && empty($saleorrent_err) ){
        ...
        $sql = "INSERT INTO tbl_sample (county, country, town, description, details, address, image, latitude, longitude, bedrooms, bathrooms, price, type, saleorrent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssssss", $param_county, $param_country, $param_town, $param_description, $param_details, $param_address, $param_image, $param_latitude, $param_longitude, $param_bedrooms, $param_bathrooms, $param_price, $param_type, $param_saleorrent);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Please fill this form and submit to add property record to the database.</p>
